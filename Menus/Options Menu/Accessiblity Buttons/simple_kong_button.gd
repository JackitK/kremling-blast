...
func _ready() -> void:
	...
	load_data()
	print("Simplify Kong state" + str(SettingsDataContainer.simplify_kong_rate))
	if SettingsDataContainer.simplify_kong_rate == true:
		check_button.set_pressed_no_signal(true)
	elif SettingsDataContainer.simplify_kong_rate == false:
		check_button.set_pressed_no_signal(false)
	set_label_text(check_button.button_pressed)

func load_data() -> void:
	on_autofire_toggled(SettingsDataContainer.simplify_kong_rate)
...
func on_autofire_toggled(button_pressed: bool) -> void:
	...
	SettingsSignalBus.emit_on_simplify_kong_rate_toogled(button_pressed)
